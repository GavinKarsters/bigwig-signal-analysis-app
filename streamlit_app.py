...
def extract_signals_fast(bigwig_file_groups, bed_file, extend=500, max_regions=5000):
    try:
        bed_df = pd.read_csv(bed_file, sep='\t', header=None, usecols=[0, 1, 2], names=['chr', 'start', 'end'], dtype={'chr': str})
...
def create_heatmap_plot(profile_dict, bed_names, bigwig_name, original_bed_names=None,
    ...
    for idx, (custom_name, original_name) in enumerate(valid_beds):
        ...
        # Sort matrix by mean signal of each row
        if sort_regions and matrix.shape[0] > 1:
            mean_signal_per_region = matrix.mean(axis=1)
            sorted_indices = np.argsort(mean_signal_per_region)[::-1] # Sort descending
            matrix = matrix[sorted_indices, :]

        # Create a divider for the main axis to add the profile plot on top
        ...
        plt.setp(ax_profile.get_xticklabels(), visible=False) # Hide x-axis labels on profile
        ...
        # Set x-axis labels to be meaningful distances
        positions = p_data['positions']
        tick_pos = np.linspace(0, len(positions)-1, 5)
        tick_labels = [f'{int(positions[int(p)]/1000)} kb' for p in tick_pos]
        ax.set_xticks(tick_pos)
        ax.set_xticklabels(tick_labels)
        ax.set_xlabel("Distance from Center")

    # Add a single colorbar for the whole figure
    fig.colorbar(im, ax=axes.ravel().tolist(), shrink=0.6, label="Signal Intensity")
...
def main():
    ...
    if uploaded_excel:
        ...
        if pre_extracted_data:
            ...
            # Display summary in columns for tidiness
            col1, col2 = st.columns(2)
    ...
    with st.sidebar:
        ...
        # Determine plot types and defaults
        if pre_extracted_data:
            plot_type_options = ["Boxplot", "Line plot", "Heatmap", "All"]
            default_index = plot_type_options.index(pre_extracted_data['analysis_params'].get('plot_type', 'All'))
        else:
            plot_type_options = ["Boxplot", "Line plot", "Heatmap", "All"]
            default_index = 3 # Default to "All" for new analysis
        ...
        if plot_type in ["Heatmap", "All"]:
            ...
            cmap_choice = st.selectbox("Colormap:", ['viridis', 'coolwarm', 'Reds', 'Blues', 'YlGnBu', 'magma'], index=['viridis', 'coolwarm', 'Reds', 'Blues', 'YlGnBu', 'magma'].index(default_cmap))
            sort_regions = st.checkbox("Sort regions by mean signal", value=default_sort)
    ...
    if st.button("🚀 Run Analysis", type="primary", use_container_width=True, disabled=not bigwig_files or not bed_files or st.session_state.analysis_running):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            try:
                ...
                analysis_params = {'plot_type': plot_type, 'y_max': y_max if 'y_max' in locals() else None, 'extend_bp': extend_bp if 'extend_bp' in locals() else None, 'max_regions': max_regions, 'line_extend': 2000, 'line_bin_size': 20, 'cmap': cmap_choice, 'sort_regions': sort_regions, 'vmin': vmin, 'vmax': vmax}
